...
class SSOClientView(View):
    ...
    def get(self, request, **kwargs):  # pylint: disable=unused-argument
        ...
        sso_url = client_util.generate_sso_url(
            nonce_val, self.sso_redirect_url, require_activation=False)
...
class SSOCreateSessionView(View):
    ...
    def get(self, request, **kwargs):  # pylint: disable=unused-argument
        ...
        nonce = client_util.get_param(user_data, u'nonce')
        if self.nonce_service.is_nonce_valid(nonce):
            try:
                ...
                self.create_user_session(request,
                                         client_util.get_param(user_data, u'email'),
                                         client_util.get_param(user_data, u'external_id'),
                                         client_util.get_param(user_data, u'username'))
                return HttpResponseRedirect(

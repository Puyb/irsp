...
class SSOClientUtils(BaseSSOGenerator):
    ...
    def generate_sso_url(
        ...
        :param return_sso_url: URL to redirect to after authentication is successful
    ...
    def decode_client_data(cls, base64_payload: str) -> dict:
        ...
        """
        # Decode payload bytes from Base64
        bytes_payload = base64.b64decode(base64_payload)
        # Decode ASCII from payload bytes (other encodings must be %-encoded)
        ascii_payload = bytes_payload.decode("ascii", "ignore")
        # Parse param and value pairs from the payload
        parsed_payload = parse_qs(ascii_payload)
